      ...
      case "week": {
        ...
        if (t === "date" || t.startsWith("daterange")) {
          ...
          if (!r.success || !s.success)
            return console.warn("[Gofakeit Autofill] Error getting year or week:", r.error || s.error), (r.status === 400 || s.status === 400) && p(e, "Failed to get random week"), { success: !1, usedFunc: "year/number?min=1&max=53" };
          const n = s.data.padStart(2, "0");
          e.value = `${r.data}-W${n}`;
        }
        break;
      }
...
async function w(e) {
  ...
  if (o === "" || o === "text") {
    ...
    if (/^[+-]?\d+$/.test(y)) {
      ...
      return `number?min=0&max=${Math.pow(10, g) - 1}`;
    }
    if (/^[+-]?\d+\.\d+$/.test(y)) {
      const g = y.replace(/[^0-9.]/g, "").split("."), b = Math.min((g[1] || "2").length, 6);
